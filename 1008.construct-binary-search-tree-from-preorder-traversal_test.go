...
func Test_bstPreRecursive(t *testing.T) {
	type args struct {
		nums  []int
		limit int
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, got1 := bstPreRecursive(tt.args.nums, tt.args.limit)
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("bstPreRecursive() got = %v, want %v", got, tt.want)
			}
			if !reflect.DeepEqual(got1, tt.want1) {
				t.Errorf("bstPreRecursive() got1 = %v, want %v", got1, tt.want1)
			}
		})
	}
}
